...
export default function RideDetailCard({
  ...
  const formattedTimeString = format(originalDate, 'HH:mm', { locale: zhTW })
          ...
          <View style={isPastRide ? styles.completeIconContainer : styles.iconContainer}>
            <View>
              {isPastRide ? (
                ...
                <Icon
                  name={rideInfo.iconName}
                  style={{ width: 24, height: 24 }}
                  fill={theme[rideInfo.iconFilledColor]}
                />
              )}
            </View>
            <View>
          ...
          <View style={isPastRide ? styles.completeTextContainer : styles.TextContainer}>
            <View style={{ flex: 1, flexDirection: 'column', gap: 10, alignItems: 'flex-start' }}>
              <Text
                style={
                  index !== ridePhase
                    ? isPastRide
                      ? styles.completePassengerInfoText
                      : styles.passengerInfoText
                    : styles.currentPassengerInfoText
                }
              >
                {isPastRide
                  ? `${rideInfo.passengerName} (${rideInfo.passengerCount}人)${rideInfo.completeType}`
                  : `${rideInfo.type} ${rideInfo.passengerName} (${rideInfo.passengerCount}人)`}
              </Text>
              <Text
                style={
                  index !== ridePhase
                    ? isPastRide
                      ? styles.completeLocationText
                      : styles.locationText
            ...
            <View style={{ alignItems: 'flex-end', marginRight: 30 }}>
              <Text style={index !== ridePhase ? styles.completeLocationText : styles.timeText}>
                {isPastRide
                  ? `${formattedTimeString}${rideInfo.passengerActionType}`
                  : `預計${formattedTimeString}${rideInfo.passengerActionType}`}
              </Text>
            </View>
          </View>
        </View>
