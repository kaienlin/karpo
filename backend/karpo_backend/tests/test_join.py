...
async def test_post_joins_and_do_action(
    ...
    if matched:
        ...
        assert resp.status_code == status.HTTP_200_OK
        try:
            get_join_status_resp_obj = GetRideIdJoinIdStatusResponse.model_validate(resp.json())
        except ValidationError:
            pytest.fail("invalid response")
        ...
        for get_joins_status in ["all", "accepted", "rejected", "pending"]:
            ...
            assert resp.status_code == status.HTTP_200_OK
            try:
                get_ride_id_join_resp_obj = GetRideJoinsResponse.model_validate(resp.json())
            except ValidationError:
                pytest.fail("invalid response")

            assert get_ride_id_join_resp_obj.num_available_seat == post_rides_req_body["num_seats"]
        ...
        for get_joins_status in ["all", "accepted", "rejected", "pending"]:
            ...
            if driver_action == "accept":
                assert get_ride_id_join_resp_obj.num_available_seat == post_rides_req_body["num_seats"] - post_requests_req_body["num_passengers"], "num of seats left not correct."
                if (get_joins_status in ["accepted", "all"]):
                    assert len(get_ride_id_join_resp_obj.joins) == 1
                else:
                    assert len(get_ride_id_join_resp_obj.joins) == 0
            if driver_action == "reject":
                assert get_ride_id_join_resp_obj.num_available_seat == post_rides_req_body["num_seats"], "num of seats left not correct."
