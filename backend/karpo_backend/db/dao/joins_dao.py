...
class JoinsDAO:
    ...
    async def put_joins_model_by_id(
        self,
        join_id: uuid.UUID,
        action: Literal["accpet", "reject"]
    ) -> Optional[JoinsModel]:
        if action == "accept":
            row_updated = self.session.query.filter(
                JoinsModel.join == join_id).update(status="accepted")
        else:
            row_updated = self.session.query.filter(
                JoinsModel.join == join_id).update(status="rejected")
